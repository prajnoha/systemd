...
static int synthesize_uevent(sd_device *dev, const char *syspath, const char *action) {
        const char *filename;
        int r;

        filename = strjoina(syspath, "/uevent");
        log_device_debug(dev, "device is closed, synthesising '%s' on %s", action, syspath);
        r = write_string_file(filename, action, WRITE_STRING_FILE_DISABLE_BUFFER);
        if (r < 0)
                return log_device_debug_errno(dev, r, "Failed to write '%s' to %s: %m", action, filename);
        return 0;
}
...
static int synthesize_change(sd_device *dev) {
        ...
        full_action = strjoin(device_action_to_string(DEVICE_ACTION_CHANGE), " ",
        ...
        if (streq_ptr("block", subsystem) &&
            ...
            !startswith(sysname, "dm-")) {
                 ...
                 */
                (void) synthesize_uevent(dev, syspath, full_action);
                ...
                FOREACH_DEVICE(e, d) {
                        ...
                        (void) synthesize_uevent(dev, s, full_action);
                }
